...
def rank_sentences(document: Document, corpus_idfs: {str: float}, rank_option: str, current_summary: [str]) -> [(str, int)]:
    ...
    for sentence_index, sentence in enumerate(sentences, start=0):
        ...
        tfidfs = get_tfidfs(sentence, sentence_index, inverted_index, corpus_idfs)

    #return sentence : score ordered by position in text

def ranking(document: Document, max_sentences: int, max_chars: int, order_ranked: bool, corpus_idfs: {str: float}, args: {str: any}):
